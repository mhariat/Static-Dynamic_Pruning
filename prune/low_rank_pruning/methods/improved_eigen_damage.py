...
class ImprovedEigenPruner(LowRankPruner):
    def __init__(self, model, prune_ratio_limit, normalize, log_interval, use_bias=True, fisher_type='true',
                 allow_increase=False, regularization=1e-3, ma=0.9, correct_eigenvalues=False, sua=False,
                 use_full_cov_a=True, decomp_method='tucker', pool_momentum=0.5, back_ratio=0.25):
        super(ImprovedEigenPruner, self).__init__(model=model, prune_ratio_limit=prune_ratio_limit, normalize=normalize,
    ...
    def _forward_conv(self, module, input):
        ...
        if self.sua | (patch_size == 1):
            a = x.view(-1, x.size(-1))
            value = (a.t() @ a)/batch_size/patch_size
            if self.steps == 0:
                self.A[module] = value
            else:
            ...
            value_c = (c.t() @ c) / batch_size / patch_size
            value_k = (k.t() @ k) / batch_size / channel_size
    ...
    def _backward_func(self, module, grad_wr_input, grad_wr_output):
        super(ImprovedEigenPruner, self)._backward_func(module, grad_wr_input, grad_wr_output)
        if isinstance(module, nn.Conv2d):
        ...
        if self.steps == 0:
            self.G[module] = value
            self.g[module] = grad_wr_output[0].data
        else:
            moving_average(self.G[module], value, self.ma)
    ...
    def _compute_saliencies(self, dataloader=None):
        super(ImprovedEigenPruner, self)._compute_saliencies(dataloader=dataloader)
        self.init_step()
        self.model.train()
    ...
    def _clear_buffers(self):
        super(ImprovedEigenPruner, self)._clear_buffers()
        self.G = OrderedDict()
        self.A = OrderedDict()
        self.Q_G = OrderedDict()
        self.Q_A = OrderedDict()
